          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','supervisor_no') || $check_field('add','supervisor_no') || $check_field('set','supervisor_no')" label="主管编号" name="supervisor_no">
                                <uni-easyinput type="text" v-model="form['supervisor_no']" v-if="user_group === '管理员' || (form['department_head_id'] && $check_field('set','supervisor_no')) || (!form['department_head_id'] && $check_field('add','supervisor_no'))" :disabled="disabledObj['supervisor_no_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','supervisor_no')">
                  {{ form['supervisor_no'] }}
                </text>
                            </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','supervisor_name') || $check_field('add','supervisor_name') || $check_field('set','supervisor_name')" label="主管姓名" name="supervisor_name">
                                <uni-easyinput type="text" v-model="form['supervisor_name']" v-if="user_group === '管理员' || (form['department_head_id'] && $check_field('set','supervisor_name')) || (!form['department_head_id'] && $check_field('add','supervisor_name'))" :disabled="disabledObj['supervisor_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" label="性别" name="gender">
                  ...
                  v-if="user_group === '管理员' || (form['department_head_id'] && $check_field('set','gender')) || (!form['department_head_id'] && $check_field('add','gender'))"
                ></uni-data-select>
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      group_table: "department_head",
      is_password: true,
      field: "department_head_id",
      url_add: "~/api/department_head/add?",
      url_set: "~/api/department_head/set?",
      url_get_obj: "~/api/department_head/get_obj?",
      url_upload: "~/api/department_head/upload?",
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/department_head/upload?'),
    ...
    submit_sub(form_sub){
      if(form_sub["department_head_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?department_head_id=" + form_sub["department_head_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/department_head/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/department_head/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/department_head/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/department_head/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/department_head/view','get');

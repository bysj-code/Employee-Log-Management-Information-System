        ...
        <view>
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','log_title') || $check_field('add','log_title') || $check_field('set','log_title')" label="日志标题" name="log_title">
                                <uni-easyinput type="text" v-model="form['log_title']" v-if="user_group === '管理员' || (form['employee_log_id'] && $check_field('set','log_title')) || (!form['employee_log_id'] && $check_field('add','log_title'))" :disabled="disabledObj['log_title_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','log_title')">
                  {{ form['log_title'] }}
                </text>
                            </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','employee_id') || $check_field('add','employee_id') || $check_field('set','employee_id')" label="员工工号" name="employee_id">
                  ...
                  v-if="user_group === '管理员' || (form['employee_log_id'] && $check_field('set','employee_id')) || (!form['employee_log_id'] && $check_field('add','employee_id'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['employee_id']"
                  :localdata="list_user_employee_id"
              ...
              <uni-forms-item v-if="$check_field('get','employee_name') || $check_field('add','employee_name') || $check_field('set','employee_name')" label="员工姓名" name="employee_name">
                                <uni-easyinput type="text" v-model="form['employee_name']" v-if="user_group === '管理员' || (form['employee_log_id'] && $check_field('set','employee_name')) || (!form['employee_log_id'] && $check_field('add','employee_name'))" :disabled="disabledObj['employee_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','record_time') || $check_field('add','record_time') || $check_field('set','record_time')" label="记录时间" name="record_time">
                        <uni-datetime-picker v-if="user_group === '管理员' || (form['employee_log_id'] && $check_field('set','record_time')) || (!form['employee_log_id'] && $check_field('add','record_time'))" v-model="form['record_time']" type="date" :disabled="disabledObj['record_time_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','content_details') || $check_field('add','content_details') || $check_field('set','content_details')" label="内容详情" name="content_details">
                        <uni-easyinput type="textarea" v-model="form['content_details']" v-if="user_group === '管理员' || (form['employee_log_id'] && $check_field('set','content_details')) || (!form['employee_log_id'] && $check_field('add','content_details'))" :disabled="disabledObj['content_details_isDisabled']" />
                <!-- 仅查看 -->
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/employee_log/upload?'),

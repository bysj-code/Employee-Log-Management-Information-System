<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="通知列表推荐"
      url="/pages/notification_list/list"
      :list="list_article"
      v-if="$check_action('/notification_list/list', 'get')"
    >
	  <list_notification_list :list="list_notification_list"></list_notification_list>
		...
		*/
		get_notification_list() {
			...
			this.$get(url, {
					...
					console.log("通知列表" ,json.result.list);
					this.list_notification_list = json.result.list;
											this.list_notification_list.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_notification_list ,"notification_list" ,"20009");
									}
			});
		},

<template>
	<view class="page_notification_list diy_detail diy_detail--new" id="notification_list_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					...
					<view v-if="$check_field('get','content_related')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>内容相关</span>
							</view>
					...
					<view v-if="$check_field('get','content_details')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>内容详情</span>
							</view>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=notification_list&source_field=notification_list_id&source_id=' + obj['notification_list_id']">
							评论
						</navigator>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					...
					source_field: "notification_list_id",
					source_id: obj["notification_list_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "notification_list",
					source_field: "notification_list_id",
					source_id: this.obj["notification_list_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/notification_list/set?notification_list_id=' + _this.obj["notification_list_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/notification_list/set?notification_list_id=' + obj["notification_list_id"], {
					hits
				}, (res) => {
					if(res.result){

						...
						<view class="register_view">
							<uni-forms class="form_register" :rules="rules" :value="form" ref="form"
								...
								<uni-forms-item class="user_group" name="user_group" label="身份">
									...
									<!-- 员工 -->
								<form_staff v-if="form.user_group === '员工'" @change="handleSubFormChange" :form="form_sub" ></form_staff>
...
<script>
	import mixin from "@/libs/mixins/page.js"
		/* 员工 */
	import form_staff from "@/components/diy/form_staff";
			...
			async submit_check(param){
				...
				if(!ret && user_group){
						...
						var form_sub = Object.assign({} ,this.form_sub);
											if(!ret && !form_sub["employee_id"]){
							ret = "员工工号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/staff/count?", {"employee_id": form_sub["employee_id"]});
							if(res.result){
								ret = "员工工号已存在!";
							}
						}

<template>
	<view class="diy_details diy_div_clock_in">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','clock_type')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>打卡类型:</span>
						</view>
							<view class="diy_field diy_text">
					...
					<view v-if="$check_field('get','clocking_time')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>打卡时间:</span>
						</view>
							<view class="diy_field diy_datetime">
							<text>
								{{ $toTime(obj["clocking_time"],"yyyy-MM-dd hh:mm:ss") }}
							</text>
						</view>
						</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/clock_in/get_obj", {
					clock_in_id:this.query.clock_in_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["clock_in_id"] !==0) {

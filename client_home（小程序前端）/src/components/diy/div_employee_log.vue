<template>
	<view class="diy_details diy_div_employee_log">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					<view v-if="$check_field('get','log_title')" class="col-12 col-md-6">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/employee_log/get_obj", {
					employee_log_id:this.query.employee_log_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["employee_log_id"] !==0) {

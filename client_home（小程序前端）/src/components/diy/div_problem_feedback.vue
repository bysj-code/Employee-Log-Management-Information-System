<template>
	<view class="diy_details diy_div_problem_feedback">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','feedback_time')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>反馈时间:</span>
						</view>
							<view class="diy_field diy_date">
							<text>
								{{ $toTime(obj["feedback_time"],"yyyy-MM-dd") }}
							</text>
						</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/problem_feedback/get_obj", {
					problem_feedback_id:this.query.problem_feedback_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["problem_feedback_id"] !==0) {

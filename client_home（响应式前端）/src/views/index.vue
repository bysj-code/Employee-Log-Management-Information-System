	...
	<div class="page_root" id="root_demo">
		            ...
		            <div class="col-12">
		                <div class="sw_no">
							...
							<div class="card_notice">
								<div class="notice_title">公告消息 <router-link to="notice/list">更多</router-link></div>
								<!-- 公告组件 -->
		...
		<div class="warp">
		    <div class="container">
				...
				<!-- 推荐通知列表模块(开始) -->
		<div class="warp" v-if="$check_action('/notification_list/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/notification_list/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_notification_list = !show_list_notification_list">
		                        <div class="switch_box">
		                            <span v-if="show_list_notification_list"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 通知列表推荐列表组件 -->
		                <list_notification_list v-if="show_list_notification_list" :list="list_notification_list" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_notification_list.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_notification_list" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_notification_list" :key="i" @click="to_details('notification_list',o,'notification_list_id')">
		                            <td v-for="(oj,n) in fields_notification_list">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	import mixin from "@/mixins/page.js";
			import list_notification_list from "@/components/diy/list_notification_list.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_article: true,
						show_list_notification_list: true,
				list_notification_list: [],
				vm_arr_notification_list: [
		...
		methods: {
			...
			get_notification_list() {
				let url = "~/api/notification_list/get_list?";
										this.$get(url, {
									"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_notification_list = json.result.list;
					}
				})
			},
		...
		computed: {
				    list_table_agency_tasks() {
		        var list = this.list_notification_list;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_notification_list.map((o) => {
		                // 第二个中括号是对象的属性

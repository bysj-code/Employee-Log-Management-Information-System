						...
						<el-form-item label="身份" prop="user_group">
							<el-select v-model="form.user_group" placeholder="身份">
								<el-option v-for="(o,i) in list_user_group" :key="i" :value="o" :label="o"></el-option>
																		...
																		<!-- 总经理 -->
						<form_general_manager v-if="form.user_group === '总经理'" :form="form_sub" ></form_general_manager>
			...
			async submit_check(param){
				...
				if(!ret && user_group){
						...
						var form_sub = Object.assign({} ,this.form_sub);
											if(!ret && !form_sub["general_manager_no"]){
							ret = "总经理编号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/general_manager/count?", {"general_manager_no": form_sub["general_manager_no"]});
							if(res.result){
								ret = "总经理编号已存在!";
							}
						}

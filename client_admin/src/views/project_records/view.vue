	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="员工工号" prop="employee_id">
													<el-select v-if="user_group === '管理员' || (form['project_records_id'] && $check_field('set','employee_id')) || (!form['project_records_id'] && $check_field('add','employee_id'))" id="employee_id" v-model="form['employee_id']" :disabled="disabledObj['employee_id_isDisabled']">
				...
				<el-form-item label="员工姓名" prop="employee_name">
							  ...
							  v-if="user_group === '管理员' || (form['project_records_id'] && $check_field('set','employee_name')) || (!form['project_records_id'] && $check_field('add','employee_name'))" :disabled="disabledObj['employee_name_isDisabled']"></el-input>
				...
				<el-form-item label="任务名称" prop="task_name">
							  ...
							  v-if="user_group === '管理员' || (form['project_records_id'] && $check_field('set','task_name')) || (!form['project_records_id'] && $check_field('add','task_name'))" :disabled="disabledObj['task_name_isDisabled']"></el-input>
				...
				<el-form-item label="分派时间" prop="dispatch_time">
												<el-input id="dispatch_time" v-model="form['dispatch_time']" placeholder="请输入分派时间"
							  v-if="user_group === '管理员' || (form['project_records_id'] && $check_field('set','dispatch_time')) || (!form['project_records_id'] && $check_field('add','dispatch_time'))" :disabled="disabledObj['dispatch_time_isDisabled']"></el-input>
				...
				<el-form-item label="完成情况" prop="completion">
								<el-select id="completion" v-model="form['completion']"
						...
						<el-option v-for="o in list_completion" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
				...
				<el-form-item label="记录时间" prop="record_time">
								<el-date-picker :disabled="disabledObj['record_time_isDisabled']" v-if="user_group === '管理员' || (form['project_records_id'] && $check_field('set','record_time')) || (!form['project_records_id'] && $check_field('add','record_time'))" id="record_time"
				...
				<el-form-item label="详情" prop="details">
								<el-input type="textarea" id="details" v-model="form['details']" placeholder="请输入详情"
						v-if="user_group === '管理员' || (form['project_records_id'] && $check_field('set','details')) || (!form['project_records_id'] && $check_field('add','details'))" :disabled="disabledObj['details_isDisabled']"></el-input>
